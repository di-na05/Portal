...
type RoleValue = "" | "Executive" | "Coach";
...
// helper to mark slots that are admin/rest periods
...
// return duration/weight for a given time slot in hours
// weekdays are always 2.5 per slot (two slots = 5h target)
// weekend slots use the individual weights described in the spec
function getSlotWeight(day: string, slot: string): number {
  if (WEEKDAY_DAYS.includes(day as (typeof WEEKDAY_DAYS)[number])) {
    return 2.5; // each weekday slot counts as 2.5h
  }

  if (["08:45 AM – 09:15 AM", "6:45 PM – 7:15 PM"].includes(slot)) {
    return 0.5; // opening/closing admin
  }
  if ([
    "11:45 AM – 12:00 PM",
    "2:30 PM – 2:45 PM",
    ...
    return 0.25; // rest slots
  }
  // all other weekend class slots
  return 1.25;
}
...
export default function ManpowerTable({ week }: ManpowerTableProps) {
  ...
  const handleBulkUpdate = (day: string, staffName: string, checked: boolean) => {
    ...
    setSelections((prev) => {
      ...
      slots.forEach((slot) => {
        const key = `${day}-${slot}-${staffName}`;
        newSelections[key] = checked ? "Executive" : "";
      });
  ...
  // returns summary for the specified days (defaults to all DAYS)
  const calculateStaffHours = (daysFilter: readonly string[] = DAYS) => {
    return COLUMNS.map((col) => {
      ...
      daysFilter.forEach((day) => {
        ...
        slots.forEach((slot) => {
          const key = `${day}-${slot}-${col}`;
          const role = isAdminSlot(slot) ? "Executive" : selections[key] ?? "";
  ...
  const weekdaySummary = calculateStaffHours(WEEKDAY_DAYS);
  const weekendSummary = calculateStaffHours(["Saturday", "Sunday"]);
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="flex h-[calc(100vh-100px)]">
                        ...
                        >
                          <label className="flex items-center justify-center">
                            <input
                              ...
                              onChange={(e) => handleBulkUpdate(day, col, e.target.checked)}
                            />
                            <span className="sr-only">Select all for {col}</span>
                          </label>
                          <span className="block mt-1">{col}</span>
                        </th>
                      ))}
                    ...
                    {getTimeSlotsForDay(day)
                      ...
                      .map((slot) => (
                        ...
                        {COLUMNS.map((col) => {
                          ...
                          const role = isAdminSlot(slot) ? "Executive" : selections[selectionKey] ?? "";
                          ...
                          return (
                            ...
                            >
                              ...
                              >
                                ...
                                <option className="text-center" value="Executive">
                                  Executive
                                </option>
        ...
        {/* Only show Update button if currently editing */}
        {isEditing && (
          <button
            type="button"
          ...
          <table className="w-full min-w-[720px] table-auto border-separate border-spacing-0 text-[0.9rem]">
            <thead>
              <tr>
                ...
                <th className="min-w-[160px] border border-[#e0e2e6] bg-gradient-to-r from-[#2c3e50] to-[#4ca1af] px-4 py-3 text-center font-semibold text-white whitespacenowrap">
                  Total (hrs)
                </th>
              </tr>
            </thead>
      ...
      <div className="mx-auto mt-12 w-full max-w-[95%] overflow-hidden rounded-[15px] border border-[#e0e2e6]/70 bg-white/90 shadow-[0_18px_45px_rgba(15,23,42,0.16)] backdrop-blur-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[720px] table-auto border-separate border-spacing-0 text-[0.9rem]">
            <thead>
              <tr>
                <th className="min-w-[80px] border border-[#e0e2e6] bg-gradient-to-r from-[#2c3e50] to-[#4ca1af] px-4 py-3 text-center font-semibold text-white whitespacenowrap">
                  No.
                </th>
                <th className="min-w-[200px] border border-[#e0e2e6] bg-gradient-to-r from-[#2c3e50] to-[#4ca1af] px-4 py-3 text-left font-semibold text-white whitespacenowrap">
                  Name
                </th>
                <th className="min-w-[140px] border border-[#e0e2e6] bg-gradient-to-r from-[#2c3e50] to-[#4ca1af] px-4 py-3 text-center font-semibold text-white whitespacenowrap">
                  Class (Coach)
                </th>
                <th className="min-w-[160px] border border-[#e0e2e6] bgGradient-to-r from-[#2c3e50] to-[#4ca1af] px-4 py-3 text-center font-semibold text-white whitespacenowrap">
                  Executive
                </th>
                <th className="min-w-[160px] border border-[#e0e2e6] bgGradient-to-r from-[#2c3e50] to-[#4ca1af] px-4 py-3 text-center font-semibold text-white whitespacenowrap">
                  Total (hrs)
            ...
            <tbody>
              {/* weekday summary rows omitted for brevity - identical structure to above */}
            </tbody>
          </table>
        </div>
      </div>
      <div className="mx-auto mt-12 w-full max-w-[95%] overflow-hidden rounded-[15px] border border-[#e0e2e6]/70 bg-white/90 shadow-[0_18px_45px_rgba(15,23,42,0.16)] backdrop-blur-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[720px] table-auto border-separate border-spacing-0 text-[0.9rem]">
            <thead>
              <tr>
                <th className="min-w-[80px] border border-[#e0e2e6] bgGradient-to-r from-[#2c3e50] to-[#4ca1af] px-4 py-3 text-center font-semibold text-white whitespacenowrap">
                  No.
                </th>
                <th className="min-w-[200px] border border-[#e0e2e6] bgGradient-to-r from-[#2c3e50] to-[#4ca1af] px-4 py-3 text-left font-semibold text-white whitespacenowrap">
                  Name
                </th>
                <th className="min-w-[140px] border border-[#e0e2e6] bgGradient-to-r from-[#2c3e50] to-[#4ca1af] px-4 py-3 text-center font-semibold text-white whitespacenowrap">
                  Class (Coach)
                </th>
                <th className="min-w-[160px] border border-[#e0e2e6] bgGradient-to-r from-[#2c3e50] to-[#4ca1af] px-4 py-3 text-center font-semibold text-white whitespacenowrap">
                  Executive
            ...
            <tbody>
              {/* weekend summary rows omitted for brevity */}
            </tbody>
          </table>
        </div>
      </div>
